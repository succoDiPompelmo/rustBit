...
impl Encode for Info {
    fn encode(&self) -> Vec<u8> {
        let files = encode_dict_entry(&"files".to_owned(), &self.files);
        let length = encode_dict_entry(&"files".to_owned(), &self.length);
        let name = encode_dict_entry(&"name".to_owned(), &self.name);
